...
def build_development_trace_projection(
    ...
    metadata: dict[str, _MetadataValue] = {
        ...
        "research_artifact_id": None if state.research is None else state.research.artifact_id,
        "plan_id": None if state.plan is None else state.plan.plan_id,
        "test_intent_ids": tuple(item.intent_id for item in state.test_intents),
